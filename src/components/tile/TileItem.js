...
export default function TileItem(props) {
  ...
  const removeSpaceFromString = (string) => {
    ...
    formattedString =
      formattedString.substring(0, 0) +
      "--" +
      string.substring(0).replace(/ /g, "");
  ...
  return (
    <>
      <div className="tile-container__panel">
          ...
          {isComponentVisible && (
            <div className="tile__menu-open">
              <ul className="tile__menu-open-list">
                <li
                  className="tile-menu-open-list__li"
                  onClick={() => {
        ...
        <span
          className={`tile__category-tope ${
            "tile__category-tope" + removeSpaceFromString(props.todo.category)
          }`}
        >
          {makeUpperCase(props.todo.category)}
        </span>
      ...
      >
        <Modal.Title>
          <i class="fas fa-exclamation-triangle fa__icon--exclamation"></i>
        </Modal.Title>
